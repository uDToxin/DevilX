...
from DEVIL.misc import sudo
from DEVIL.plugins import ALL_MODULES
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("DEVIL.plugins" + all_module)
    LOGGER("DEVIL.plugins").info("𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲🥳...")
    await userbot.start()
    await DAXX.start()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("DEVIL").info("𝗦𝗧𝗢𝗣 𝗗EV 𝗠𝗨𝗦𝗜𝗖🎻 𝗕𝗢𝗧..")
